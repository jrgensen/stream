...
import (
	...
	iface "github.com/jrgensen/stream"
...
var (
	_ iface.Stream = &stream{}
	//_ StreamStatistics             = &stream{}
	_ iface.Subscription = &subscription{}
)
...
func (s *stream) MessageFunc() iface.MessageFunc {
	return func(subject iface.Subject) iface.MutableMessage {
		m := NewMessage()
...
func (s *stream) Publish(m iface.Message) error {
...
func (s *stream) LastMessage(subject iface.Subject) (*message, error) {
...
func (s *stream) Subscribe(subjects []iface.Subject, h iface.MessageHandler) (iface.Subscription, error) {
...
func createMessage(msg jetstream.Msg) (*message, error) {
	...
	m := &message{
		...
		time:          data.Time,
		subject:       subject.FromStr(msg.Subject()),
		body:          data.Body,
		meta:          data.Meta,
	}
	return m, nil
